...
export const ProxyMonitor: React.FC<ProxyMonitorProps> = ({ className }) => {
    ...
    const loadData = async () => {
        try {
            ...
            if (config && config.proxy) {
                ...
                // await apiClient.setProxyMonitorEnabled(config.proxy.enable_logging); // Backend might not support setting this explicitly without saveConfig, but UI expects it.
            }

            const history = await apiClient.getProxyLogs(); // API client needs to support limit arg if added
            if (Array.isArray(history)) setLogs(history);

            const currentStats = await apiClient.getProxyStats();
    ...
    const toggleLogging = async () => {
        ...
        try {
            ...
            if (config && config.proxy) {
                ...
                // await apiClient.setProxyMonitorEnabled(newState);
    ...
    useEffect(() => {
        ...
        // Since we don't have Tauri events, we poll for logs.
        // Or implement SSE if the bridge supports it.
        // For minimal refactor, polling every 2-5s is acceptable for a "Web Bridge".
        const poll = async () => {
             // In a real app, optimize this to fetch only new logs.
             // Here we just reload all for simplicity.
             try {
                 const history = await apiClient.getProxyLogs();
                 if(Array.isArray(history)) setLogs(history);
